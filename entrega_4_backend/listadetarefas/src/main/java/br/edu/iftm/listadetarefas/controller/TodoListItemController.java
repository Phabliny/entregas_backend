...
class TodoListItemController {
    ...
    public ResponseEntity<TodoListItem> getById(@PathVariable("id") Integer id) {
        ...
        if (existingItemOptional.isPresent()) return ResponseEntity.status(HttpStatus.OK).body(existingItemOptional.get());
        else return ResponseEntity.status(HttpStatus.NOT_FOUND).build();
    }

    @PostMapping
    public ResponseEntity<TodoListItem> create(@RequestBody TodoListItem todoListItem, @PathVariable("idList") Integer id) {
    ...
    public ResponseEntity<TodoListItem> update(@PathVariable("id") Integer id,
        ...
        if (todoListItem != null) return ResponseEntity.status(HttpStatus.OK).body(todoListItem);
        else return ResponseEntity.status(HttpStatus.NOT_FOUND).build();
    }
    ...
    public ResponseEntity<HttpStatus> delete(@PathVariable("id") Integer id) {
        try {
            ...
            throw new ResponseStatusException(HttpStatus.NOT_FOUND, "Arquivo não encontrado!", err);
        }
    }
}